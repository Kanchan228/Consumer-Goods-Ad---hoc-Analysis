...
SELECT 
    ...
    ROUND((b.unique_product_2021-a.unique_product_2020) * 100/a.unique_product_2020,2)  AS percentage_change
FROM(
   SELECT 
...
SELECT
   segment,
   COUNT(DISTINCT product_code) AS no_of_product
FROM dim_product
GROUP BY  segment
...
(SELECT 
p.segment, 
COUNT(DISTINCT s.product_code) AS no_product_2021 
FROM fact_sales_monthly s 
...
GROUP BY  month_name, s.fiscal_year, year,c.customer;
-------------------------------------------------------------------
